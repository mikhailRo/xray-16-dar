...
void CRender::Render()
{
    ...
    {
        // This branch used to return WITHOUT rebinding base_rt, unlike the sibling branch above -
        // whatever RT was left bound from the end of the previous frame (e.g. a G-buffer target
        // written by the last loading-screen/precache frame) would stay on the backbuffer for
        // however long nothing else rebinds it. Rebind explicitly, same as the sibling branch, so
        // the presented backbuffer is always the correct target.
        Target->u_setrt(RCache, Device.dwWidth, Device.dwHeight, Target->get_base_rt(), 0, 0, Target->get_base_zb());
